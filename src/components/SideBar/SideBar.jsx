...
function SidebarItem({ icon: Icon, label, active, onClick }) {
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-2 px-4 py-2 rounded-md w-full text-left transition ${
...
export default function Sidebar({ isOpen, setIsOpen, activeTab }) {
  ...
  return (
    <aside
      ...
      } fixed lg:static top-0 left-0 z-50 w-64 bg-white border-r min-h-screen p-4`}
    >
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-bold text-purple-600">WiSch⚫⚫l</h2>
        <button
